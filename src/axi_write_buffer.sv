...
module axi_write_buffer #(
  ...
  stream_fifo #(
      ...
      .data_o(mst_req_o.w),  // this attaches also the w.last channel!
  ...
  assign last_pop = w_pop && mst_req_o.w.last;  // mst_req_o.w.last is set by the queue
...
`endif
      ...
      WaitingSync: begin
        ...
        if (w_state_q == WaitingSync) begin
`ifdef DEBUG_AXI_WRITE_BUFFER
          $display("[AXI_WRBUF][%0t] AW: Last popped. last_stored=%0d, last_not_full=%0b", $time,
                   last_stored, last_not_full);
`endif
          aw_state_d = WaitingInput;
